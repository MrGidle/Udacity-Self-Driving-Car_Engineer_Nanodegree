...
vid = cv2.VideoCapture('solidYellowLeft.mp4')
...
while (vid.isOpened()):
    ...
    if ret == True:
        ...
        def draw_line_robust(img, lines, color, thickness=10):
            ...
            imshape = img.shape
            ysize = imshape[0]
            ytop = int(0.62 * ysize)  # need y coordinates of the top and bottom of left and right lane
            ybtm = int(ysize)  # to calculate x values once a line is found

            for line in lines:
                for x1, y1, x2, y2 in line:
                    slope = float(((y2 - y1) / (x2 - x1)))
                    if (slope < -0.3):  # if the line slope is greater than tan(26.52 deg), it is the left line
                        x_left.append(x1)
                        x_left.append(x2)
                        y_left.append(y1)
                        y_left.append(y2)
                    if (slope > 0.3):  # if the line slope is less than tan(153.48 deg), it is the right line
                        x_right.append(x1)
                        x_right.append(x2)
                        y_right.append(y1)
                        y_right.append(y2)
